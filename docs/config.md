...
You can set `php` to any version that is available in our [php service](https://docs.lando.dev/php). However, you should consult the [Symfony requirements](https://symfony.com/doc/current/setup.html) to make sure that version is actually supported by Symfony itself.
...
If you are unsure about how to configure the `cache` we *highly recommend* you check out our [redis](https://docs.lando.dev/redis) and [memcached](https://docs.lando.dev/memcached)) docs as well as the [Symfony ones](https://symfony.com/doc/current/cache.html).
